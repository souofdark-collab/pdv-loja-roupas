...
export default function Vendas() {
  ...
  const [trocaForm, setTrocaForm] = useState(null); // venda_id to create return/exchange
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    await window.api.put(`/api/vendas/${editingVenda}`, editForm);
  ...
  return (
    <div>
      ...
      <div className="card">
        <table>
          <thead>
            <tr><th>#</th><th>Data</th><th>Cliente</th><th>Vendedor</th><th>Pagamento</th><th>Status</th><th>Total</th><th>Ações</th></tr>
          </thead>
          <tbody>
            {vendas.map(v => (
              <tr key={v.id}>
                ...
                <td style={{ display: 'flex', gap: 4 }}>
                  <button className="btn-secondary" style={{ padding: '4px 10px', fontSize: 12 }} onClick={() => setSelectedVenda(v)}>
                    Ver
                  </button>
                  <button className="btn-warning" style={{ padding: '4px 10px', fontSize: 12 }} onClick={() => openEdit(v)}>
                    Editar
                  </button>
                  {v.status === 'finalizada' && (
                    <button className="btn-danger" style={{ padding: '4px 10px', fontSize: 12 }} onClick={() => setTrocaForm(v)}>
                      Troca/Dev.
                    </button>
                  )}
                </td>
