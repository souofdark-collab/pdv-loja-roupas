...
const allMenuItems = [
  ...
  { path: '/estoque', icon: '📦', label: 'Estoque', roles: ['admin', 'caixa'], alertKey: 'estoque' },
...
export default function Layout({ children, user, onLogout }) {
  ...
  const [estoqueBaixoQtd, setEstoqueBaixoQtd] = useState(0);

  useEffect(() => {
    const fetchAlerts = () => {
      window.api.get('/api/relatorios/estoque-minimo').then(items => setEstoqueBaixoQtd(items.length)).catch(() => {});
    };
    fetchAlerts();
  ...
  const getAlert = (item) => {
    if (item.alertKey === 'estoque' && estoqueBaixoQtd > 0) return estoqueBaixoQtd;
    return 0;
  };
      ...
      <aside style={{
        ...
        <nav style={{ flex: 1, padding: '8px 0', overflowY: 'auto' }}>
          {allMenuItems.filter(item => item.roles.includes(user.cargo)).map(item => {
            ...
            const alertCount = getAlert(item);
            return (
              <Link
                key={item.path}
                to={item.path}
